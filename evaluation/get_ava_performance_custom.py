...
def combine_res_with_size(result_list):
  ...
  # scale mAPs with *100 for readability
  res_dict = {r[0].split('/AP@0.5IOU/')[-1]:100*r[1] for r in result_list}
  # {u'PascalBoxes_Precision/mAP@0.5IOU': 0.16426229802003,
  # u'answer phone': 0.4416065942965194,
  # u'bend/bow (at the waist)': 0.29166019646294344,
  # u'carry/hold (an object)': 0.4682357576724985,
  ...
  # {u'answer phone': 3251,
  #  u'bend/bow (at the waist)': 8148,

  cls_strs = tr_sizes.keys()

  combined = [(cls_str, tr_sizes[cls_str], val_sizes[cls_str], res_dict[cls_str]) for cls_str in cls_strs]
  mAP = res_dict['PascalBoxes_Precision/mAP@0.5IOU']

  # sort by training size,ie index 1
  combined.sort(key=lambda x: x[1])

  print('\n')
  print("{:<40}{:<15}{:<15}{}".format('Action Class', 'Train Size', 'Val Size', 'Class AP'))
...
def main():
  ...
  result_list = metrics.items()
  result_list.sort(key=lambda x:x[0])

  combine_res_with_size(result_list)

  detection_filename_ext = os.path.basename(vars(args)['detections'].name)
  detection_name = detection_filename_ext.split('.')[0]

  output_file_path = '%s/%s.json' % (RESULTS_FOLDER, detection_name)
  with open(output_file_path, 'w') as fp:
      json.dump(result_list, fp, indent=2)
  print('Results are saved to %s' % output_file_path)
